...
session_start();
include '../../connect.php';
if (!$_SESSION['email'] && !$_SESSION['cntrl'] && $_SESSION['cnt'] != $_GET['target']) {
...
<head>
    <meta charset="utf-8" />
    <title>View Your Note Record - Wikkinote</title>
...
<body>
    <?php
    $sql = "SELECT * FROM wikkinote WHERE noteid = '$noteid' AND user_email = '$mymail';";
    ...
    ?>
        ...
        <div id="container">
            ...
            <p id="h1"><img class="imgs" src="../../../../assets/wikki.svg"><u>WikkiNote</u> - View your <?php echo $row['subjects']; ?> Note:</p>

            <form action="" method="post">
                <fieldset>
                    <label for="subject">Subject:</label>
                    <input type="text" id="subject" value="<?php echo $row['subjects']; ?>" readonly name="subject" />

                    <label for="topic">Topic:</label>
                    <input type="text" id="topic" value="<?php echo $row['topic']; ?>" readonly name="topic" />

                    <label for="note" style="margin-bottom: -15px;">Note/Summary:</label>
                    <textarea id="note" name="note" readonly><?php echo $row['content']; ?></textarea>

                    <button onclick="lecture('<?php echo $row['topic'] . ': ' . $row['content']; ?>')" style="background-color:rgb(33, 33, 63); color:white;" class="btn mt-4"><i class="bi bi-mic pe-1"></i> Lecture</button>
                </fieldset>
            </form>
        <?php } ?>
